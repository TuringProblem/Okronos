	...
	public boolean deathByTarget(Player pKiller) {
		if (target != null) {
			...
			if (pKiller != null && pKiller.getUserId() == target.getUserId()) {
				...
				pKiller.UpdateBountyPoints(1);
				pKiller.sendMessage("<shad=000000><col=BA0000>[Bounty Points]</shad> <col=0C2AC1>You earn <col=C10CB9>(1) pk points <col=0C2AC1>after killing <col=0C2AC1>"
					+ player.getName() + "<col=0C2AC1>! You now have <col=0C2AC1>" + pKiller.GetBountyPoints() + " bounty points<col=0C2AC1>!");
...
//                emblem.setId(emblem.getId() == 12746 ? 12748 : emblem.getId() + 1);
	...
	private void updateTargetInfo() {
		...
		VarPlayerRepository.BOUNTY_HUNTER_EMBLEM.set(player, highestEmblem == null ? 0 : Math.max(1, highestEmblem.getId() - 12746));
	}
