...
/**
 * Grace — sells graceful clothing in exchange for marks of grace.
 */
@PluginHandler
public class Grace {

    private static final int GRACE = 5919;

    static {
        NPCAction.register(GRACE, "talk-to", (player, npc) ->
                player.dialogue(
                        new NPCDialogue(npc, "Hello, " + player.getName() + ". Would you like to see the mark exchange, or learn more about marks of grace?"),
                        new OptionsDialogue(
                                new Option("Open shop", () -> ShopManager.openIfExists(player, "")),
                                new Option("Learn more", () -> {
                                    if (!player.insideWildernessAgilityCourse) {
                                        player.dialogue(new NPCDialogue(npc,
                                                "While practicing on rooftop agility courses, you will occasionally encounter marks of grace.<br>"
                                                + "Bring them to me and we can trade.<br>My outfit will allow you to run longer distances and recover more quickly."));
                                    } else {
                                        player.dialogue(
                                                new NPCDialogue(npc, "While completing this wilderness course, you'll be rewarded anywhere from 1 to 3 marks of grace along with a little blood money."),
